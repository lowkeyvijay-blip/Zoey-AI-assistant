    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto px-4 py-3 flex flex-col gap-1.5">
        ...
        {data?.map((n) => (
          ...
          >
            <p className="text-[13px] text-[var(--color-text-primary)] leading-snug">{n.title}</p>
            {n.message && (
              <p className="text-[12px] text-[var(--color-text-secondary)] mt-0.5">{n.message}</p>
            )}
            <p className="text-[11px] text-[var(--color-text-tertiary)] mt-1">{n.created_at}</p>
          </div>
        ))}
      </div>
    </div>
